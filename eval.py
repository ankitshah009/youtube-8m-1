...
if __name__ == "__main__":
  ...
  flags.DEFINE_boolean("force_output_model_name", False, "If true, force model name to be 'inference_model', for model submission")    
  flags.DEFINE_boolean("create_meta_only", False, "If true, only create meta graph without evaluation on data")
...
def get_input_evaluation_tensors(reader,
  ...
  with tf.name_scope("eval_input"):
    ...
    # note that validate file names are different on gcloud and locally    
...
def build_graph(reader,
  ...
  with tf.variable_scope("tower"):
    result = model[0].create_model(model_input,
    ...
    if FLAGS.ensemble_num> 1:
        ...
        for ensemble_idx in range(1,FLAGS.ensemble_num):
            with (tf.variable_scope("model"+str(ensemble_idx))):                                
                result2 = model[ensemble_idx].create_model(model_input,
                                            num_frames=num_frames,
                                            vocab_size=reader.num_classes,
                                            labels=labels_batch,
                                            is_training=False,
    ...
    if FLAGS.ensemble_num==1:
      ...
      predictions = 0
      for ensemble_idx in range(FLAGS.ensemble_num):
...
def evaluation_loop(video_id_batch, prediction_batch, label_batch, loss,
  ...
  with tf.Session(config=config) as sess:
    ...
    if latest_checkpoint:
      ...
      global_step_val = os.path.basename(latest_checkpoint).split("-")[-1].split('_')[-1]
      ...
      if FLAGS.force_output_model_name:
          ...
          selected_collections = ['global_step',
                                 'input_batch',
                                 'input_batch_raw',
                                 'labels',
                                 'local_variables',
          ...
          tf.train.export_meta_graph(
            filename=os.path.join(FLAGS.train_dir, "inference_model.meta"),
          ...
          if "ensemble" in FLAGS.checkpoint_file:
            saver.save(sess, os.path.join(FLAGS.train_dir, FLAGS.checkpoint_file.replace('ensemble','ensemble_'+str(FLAGS.ensemble_wts).replace(',','').replace(' ','_').replace('.','').replace('[','').replace(']',''))))
      else:
          if "avg" not in FLAGS.checkpoint_file:
            saver.save(sess, os.path.join(FLAGS.train_dir, "inference_model_" + latest_checkpoint.split('-')[-1]))
          else: 
            saver.save(sess, os.path.join(FLAGS.train_dir, "inference_model_" + FLAGS.checkpoint_file))
    else:
