...
flags.DEFINE_string("checkpoints_start_end", "",
                    "Comma-separated pair of checkpoints between which to be averaged")
...
def main(_):
  ...
  elif FLAGS.checkpoints_start_end:
    checkpoints = [c.strip() for c in FLAGS.checkpoints_start_end.split(",")]
    print(checkpoints)
    assert len(checkpoints) == 2
    start_number, end_number = int(checkpoints[0]), int(checkpoints[1])

    files = gfile.Glob(FLAGS.prefix+'*index')
    checkpoints = [extract_number(os.path.basename(x)) for x in files]
    checkpoints = [FLAGS.prefix + x for x in checkpoints \
                   if (int(x)>=start_number) and (int(x)<=end_number)]
  ...
  print("Average the following checkpoints:\n",checkpoints)
  if not checkpoints:
    if FLAGS.checkpoints:
      raise ValueError(
  ...
  print("Reading variables and averaging checkpoints:")
  ...
  print("Averaged checkpoints saved in %s", FLAGS.output_path)
