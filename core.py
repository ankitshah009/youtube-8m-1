# https://github.com/tensorflow/tensorflow/blob/master/tensorflow/python/layers/core.py
...
class Dense(base.Layer):
  ...
  def build(self, input_shape):
    ...
    self.kernel = self.add_variable('kernel',
                                    ...
                                    dtype=dtypes.float16 if self.float16_flag else self.dtype,
                                    trainable=True)
    if self.use_bias:
  ...
  def call(self, inputs):
    ...
    if len(shape) > 2:
      ...
      outputs = gen_math_ops.mat_mul(inputs, math_ops.cast(self.kernel,dtypes.float32) if self.float16_flag else self.kernel)
    if self.use_bias:
